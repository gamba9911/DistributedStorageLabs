...
def store_file(file_data, max_erasures, send_task_socket, response_socket):
    ...
    The erasure coding part codes are the customized version of the
    'encode_decode_using_coefficients'
    example of pyerasure, where you can find a detailed description of each step.
    ...
    for i in range(STORAGE_NODES_NUM):
        ...
        send_task_socket.send_multipart([task.SerializeToString(), symbol])
...
def get_file(coded_fragments, max_erasures, file_size, data_req_socket, response_socket):
    ...
    for _ in range(len(fragnames)):
        ...
        symbols.append(
            {"chunkname": result[0].decode("utf-8"), "data": bytearray(result[1])}
        )
    ...
    decoder = pyerasure.Decoder(
        field=pyerasure.finite_field.Binary8(), symbols=symbols_num, symbol_bytes=symbol_size
    )
    ...
    for symbol in symbols:
        ...
        decoder.decode_symbol(symbol["data"], bytearray(coefficients[:symbols_num]))
